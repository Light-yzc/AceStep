...
try:
    from acestep.models.sft.apg_guidance import adg_forward, apg_forward, MomentumBuffer
except ImportError:
    from apg_guidance import adg_forward, apg_forward, MomentumBuffer

# Copied from acestep/models/sft/configuration_acestep_v15.py
...
class AttentionPooler(AceStepPreTrainedModel):
    ...
    def forward(self,
        ...
        **flash_attn_kwargs: Unpack[FlashAttentionKwargs],
        ...
        x = torch.cat([special_tokens, x], dim=2) #b, t, 6, d
...
class AceStepAudioTokenizer(AceStepPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.quantizer = ResidualFSQ(
            dim=config.hidden_size, # Use hidden_size to match AttentionPooler output
...
class AceStepConditionEncoder(AceStepPreTrainedModel):
    ...
    def forward(
        ...
        # Encode timbre from reference audio #mini ver no ref 
        ...
        encoder_hidden_states, encoder_attention_mask = lyric_hidden_states, lyric_attention_mask
...
def test_forward(model, seed=42):
    ...
    # Simulate backward pass to check training memory usage
    loss.backward()
    if torch.cuda.is_available():
        max_mem = torch.cuda.max_memory_allocated() / 1024**3
        print(f"Training Max Peak Memory: {max_mem:.2f}GB (with backward)")
    ...
    for seconds in lengths_seconds:
        ...
        if total_time is not None:
            ...
            if torch.cuda.is_available():
                ...
                print(f"GPU Memory: Current={cur_mem:.2f}GB, Max Peak={max_mem:.2f}GB")
                torch.cuda.reset_peak_memory_stats()
        else:
...
if __name__ == "__main__":
    ...
    # Initialize from config directly since we modified the architecture
